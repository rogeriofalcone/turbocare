...
/*
	When the "Save and Close" button is pressed
	Check to make sure we have data to process
*/
inv.CheckSave = function(dom_obj){
	var elems = getElementsByTagAndClassName("DIV",null,"PurchaseOrders");
	if (elems.length == 0) {
		alert("Please select vendors for items before proceeding (be careful, God is watching you).");
		dom_obj.stop();
	}
}
/*
	After selecting the CatalogItem, load the vendors and thier current
	prices for that catalog item
	NOTE: changed this to use the Mochikit signal system
*/
inv.loadVendors_old = function(id){
...
inv.loadVendors = function(dom_obj){
	var elem = dom_obj.src();
	if (hasParent(elem.id,"PurchaseOrders") != true) {
		//Select the item (exclusive) make it the only orange catalog item
		var elems = getElementsByTagAndClassName(null,"listingrow_item",document);
		for (var i=0;i<elems.length;i++){
			swapElementClass(elems[i],"listingrow_item","listingrow");
		}
		swapElementClass(elem,"listingrow","listingrow_item");
		//Get our CatalogItemID
		var inputs = getElementsByTagAndClassName('INPUT',null,elem);
		for (var i=0;i<inputs.length;i++){
			if (inputs[i].name == 'CatalogItem') {
				var CatalogItemID = inputs[i].value;
			}
		}		
		//Load the vendors
		inv.toggle_message("Loading...");
	  	var d = postJSON("PurchaseOrderGetVendorsForItem","CatalogItemId="+CatalogItemID);
	  	d.addCallbacks(inv.renderVendors,inv.error_report);
  	}
}
...
inv.removeItem = function(id){
	...
	var newCatalogItem = catalogitem.cloneNode(true);
	catalogitems.appendChild(newCatalogItem);
	...
	connect(newCatalogItem,"ondblclick",inv.loadVendors);
}

inv.showPOVendor = function(dom_obj){
	...
	// We either get the item by passing an id, or the object which we double clicked
	if (dom_obj.src!=null) {
		var elem = dom_obj.src();
	} else {
		var elem = getElement(dom_obj);
	}
...
inv.moveItem = function(dom_obj){
	var vendor = dom_obj.src();
	...
	if ((vendor == null) || (catalogitem == null)){
		...
		if (getNodeAttribute(vendor_po,"class") == null) {
			...
			var vendor_po = createDOM('DIV',{'id':'PO_'+vendor.id, 'class':'listingrowcontain'},VendorName); //, 'onclick':'inv.showPOVendor("PO_'+vendor.id+'")'
		...
		inv.clearVendors();
		// Attach our vendor event
		connect(vendor_po,"onclick",inv.showPOVendor);
	}
}

inv.selectVendor = function(dom_obj){//id
	...
	// FINALLY move the item over to the selected list
	inv.moveItem();
}
...
inv.renderVendors = function(data){
	...
	// Make a box telling us there are no quotes for the item
	if (data.items.length == 0) {
		vendors.appendChild(createDOM('DIV',{'class':'listingrow_error'},"There are no quotes for this item from any vendor"));
	}
	swapDOM('vendors',vendors);
	// Attach events to our vendor records
	var vendors = getElementsByTagAndClassName('DIV',"listingrow","vendors");
	for (i=0;i<vendors.length; i++){
		connect(vendors[i],"ondblclick",inv.moveItem);
	}	
}
...
*/
inv.OpenOnLoad = function() {
	// Attach events to our catalog items
	var catalogitems = getElementsByTagAndClassName('DIV',"listingrow",document);
	for (i=0;i<catalogitems.length; i++){
		connect(catalogitems[i],"ondblclick",inv.loadVendors);
	}
...
//		connect(dateInputs[i],"onclick",inv.DatePick);
//		connect(dateInputs[i],"onkeydown",inv.DatePick);
//	}
}
/*
	Mochikit signal system.  Using this instead of the javascript "onclick" etc... events
*/
connect(window, 'onload', function(){
		inv.OpenOnLoad();
		if (getElement("btnSave")!=null) {
			connect("btnSave",'onclick',inv.CheckSave);
		}
//		if (getElement("PostOffice")!=null) {
//			connect("PostOffice",'onclick',inv.CityTownSearchKeyDown);
//		}
});
