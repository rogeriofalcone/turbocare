...
class Root(controllers.RootController):
    ...
    pharmacy_main = controllers_dispensing.Dispensing(2) # id=2 is Pharmacy Main 
    pharmacy_main = identity.SecureObject(pharmacy_main, identity.has_permission('pharmacy_main_view'))
    warehouse_main = controllers_dispensing.Dispensing(1) # id=1 is warehouse
    warehouse_main = identity.SecureObject(warehouse_main,identity.has_permission('warehouse_main_view'))

    # xray = controllers_dispensing.Dispensing(9) # id=9 is the xray dept.
    
    # Store locations - Warehouse managing tools
    pharmacy_store = controllers_store.Store(2, "pharmacy_store")
    pharmacy_store = identity.SecureObject(pharmacy_store,identity.has_permission('pharmacy_store_view'))
    ...
    @expose(template="care2x.templates.welcome")
    def index(self):
        import time
        log.debug("Happy TurboGears Controller Responding For Duty")
        return dict(now=time.ctime())

    @expose(template="care2x.templates.login")
