...
<head>
    ...
    <title>${getattr(self,'title','Patient Type Editor')}</title>
    ...
    <SCRIPT SRC="/static/javascript/config_PatientTypeEditor.js" TYPE="text/javascript"></SCRIPT>
...
<body>
	<DIV style="position:relative; width:100%; left:0px; font-size:12px" class="divtable">
		<div class="row">
			...
			<div>
				<form name='PatientTypeEditorForm' action="PatientTypeEditorSave" method="post">
					...
					<div id="part1"  style="position:relative; width:600px; left:0px; font-size:12px; display:table" class="divtable_input">
						...
						<div class="row">
							<div class="label">Short Name (no spaces, all lower case)</div>
							<div><INPUT name="ClassId" id="ClassId" type="text" value="${ClassId}" size="40" />
							</div>
						</div>
						<div class="row">
					...
					<div id="buttons" style="display:table" class="topbuttons">
						<input id="PatientTypeID" type="hidden" name="PatientTypeID" value="${PatientTypeID}" />
