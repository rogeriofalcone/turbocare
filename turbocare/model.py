...
class ClassInsurance(SQLObject):
	...
	Encounters = MultipleJoin("Encounter",joinColumn="insurance_class_nr")
...
class Personell(SQLObject):
	...
	JobTypeNr = IntCol(dbName='job_type_nr',default=0)
	...
	ContractClass = StringCol(length=35,dbName='contract_class',default='')
	...
	PayClass = StringCol(length=25,dbName='pay_class',default='')
	PayClassSub = StringCol(length=25,dbName='pay_class_sub',default='')
	LocalPremiumId = StringCol(length=5,dbName='local_premium_id',default='')
	TaxAccountNr = StringCol(length=60,dbName='tax_account_nr',default='')
	IrCode = StringCol(length=25,dbName='ir_code',default='')
	...
	Encounters = MultipleJoin("Encounter",joinColumn="current_att_dr_nr")
	Status = StringCol(length=25,default='')
	History = StringCol(length=255,default='')
...
class Department(SQLObject):
	...
	BloodTestRequests = MultipleJoin("TestRequestBlood",joinColumn='dept_nr')
	Wards = MultipleJoin("Ward",joinColumn='dept_nr')
	MedOrdercatalogs = MultipleJoin("MedOrdercatalog",joinColumn='dept_nr')
	MedOrderlists = MultipleJoin("MedOrderlist",joinColumn='dept_nr')
	MedReports = MultipleJoin("MedReport",joinColumn='dept')
	EncounterOps = MultipleJoin("EncounterOp",joinColumn='dept_nr')
	PersonellAssignments = MultipleJoin("PersonellAssignment",joinColumn='location_nr')
	Phones = MultipleJoin("Phone",joinColumn='dept_nr')
	PharmaOrderlists = MultipleJoin("PharmaOrderlist",joinColumn='dept_nr')
...
class InsuranceFirm(SQLObject):
	...
	AddrEmail = StringCol(length=60,dbName='addr_email',default='')
	PhoneMain = StringCol(length=35,dbName='phone_main',default='')
	PhoneAux = StringCol(length=35,dbName='phone_aux',default='')
	FaxMain = StringCol(length=35,dbName='fax_main',default='')
	FaxAux = StringCol(length=35,dbName='fax_aux',default='')
	ContactPerson = StringCol(length=60,dbName='contact_person',default='')
	ContactPhone = StringCol(length=35,dbName='contact_phone',default='')
	ContactFax = StringCol(length=35,dbName='contact_fax',default='')
	ContactEmail = StringCol(length=35,dbName='contact_email',default='')
	UseFrequency = IntCol(dbName='use_frequency',default=0)
	Status = StringCol(length=25,default='')
	History = StringCol(length=255,default='')
	ModifyId = StringCol(length=35,default=cur_user_id())#varchar(35) NOT NULL default '',
	ModifyTime = DateTimeCol(default=cur_date_time())
...
class TypeDepartment(SQLObject):
	...
	LdVar = StringCol(length=35,dbName='LD_var',default='')
	Description = StringCol(length=255,dbName='description')
	Departments = MultipleJoin("Department", joinColumn='type')
	Status = StringCol(length=25,default='')
	ModifyId = StringCol(length=35,default=cur_user_id())#varchar(35) NOT NULL default '',
	ModifyTime = DateTimeCol(default=cur_date_time())
	CreateId = StringCol(length=35,default=cur_user_id())#varchar(35) NOT NULL default '',
