...
class SideWalk(object):
    ...
    def remote_host(self):
        try:
            return cherrypy.request.headers.get("X-Forwarded-For",cherrypy.request.remoteHost).split(",")[-1].strip() 
        except:
            return ''

    def check_access(self):
        """Check if the requests remoteAddress is in the allowedHosts list.
           If not then redirect the user to a no access page
        """
        if self.allowedHosts == None: return
        ip = self.remote_host()
        for x in self.allowedHosts:
            if self.match_ip(x,ip):return
    ...
    def noacces(self):
        """Page shown if the user fails to authenticate/authorize"""
        return """<h3>No access for %s</h3>
                            <p>
                               By default only localhost (127.0.0.1) 
                               has access to CatWalk.
                            </p>
                            <p>
                               You can provide access to your client by passing 
                               your host address to CatWalk as a parameter. Ex:
                            </p>
                            <pre>
                                from turbogears.catwalk import CatWalk
                                catwalk = CatWalk(model,allowedHosts=['127.0.0.1','%s'])
                            </pre>
                     """% (cherrypy.request.remoteAddr,cherrypy.request.remoteAddr)
    noacces = turbogears.expose(format="json")(noacces)
    ...
    def load_object(self,object_name):
        ...
        try:
            ...
            msg='Failed to get reference to object %s'% object_name
            raise cherrypy.HTTPRedirect(turbogears.url('error', msg=msg))
        return object

    def load_instance(self,object_name,id):
        """Return an instance of the named object with the requested id"""
        object = self.load_object(object_name)          
        try:
            return object.get(id)                    
        except:
            msg='Failed to get instance of object:%s with id:%s'% (object_name,id)
            raise cherrypy.HTTPRedirect(turbogears.url('error', msg=msg))
    ...
    def add(self,**v):
        ...
        return self.instance(objectName,'%s'% new_object.id)
    ...
    def uppercase_related_join_method(self,name,method):
        """SQLObject automatically camelcase related joins methods, so we have to
           be shure that even if the users model class is lowercase it's join methods
           still should be camelcased
        """
    ...
    def updateJoins(self,objectName,id,join,joinType,joinObjectName,joins):
        ...
        obj = self.load_object(objectName)
        inst = obj.get(id)
        #drop all related joins
        for j in list(getattr(inst,join)):
            method = self.uppercase_related_join_method(joinObjectName,'remove') 
            getattr(inst,method)(j)

        #add the new joins
        joinObject = self.load_object(joinObjectName)
        joins = joins.split(',')
        for j in joins:
            try:int(j) 
            except:continue
            method = self.uppercase_related_join_method(joinObjectName,'add')
            getattr(inst,method)(joinObject.get(j))
            
        return self.instance(objectName,id); 
    ...
    def get_model(self, model_name):
        ...
        for m in dir(self.model):
            if m in ('SQLObject','InheritableSQLObject'): continue
            c = getattr(self.model,m)
            if isinstance(c, type) and issubclass(c,sqlobject.SQLObject) and (m == model_name): objs.append(m)
        return self.order_models(objs)
    ...
    index = turbogears.expose(template='care2x.templates.catwalk')(index)
    
    @turbogears.expose(template='care2x.templates.catwalk')
    def oneclass(self):
    	self.check_access()
    	return dict(models=self.get_model('BillingItem'))
