...
*/
store.RoomOnChange = function(dom_obj){
	store.toggle_message("Searching...");
	var postVars = 'WardID='+getElement('Ward').value+'&RoomNr=' + getElement('Room').value;
	var d = postJSON('RegistrationBedsInRoom',postVars);
	d.addCallbacks(store.RenderBeds,store.error_report);
}
...
/* 	Render the AddressCityTown search screen
	...
	search results
*/
store.RenderCityTownSearch = function(data){
	...
	//Close link
	var close_link = createDOM('A',{'href':'javascript:store.CloseCityTownSearch()'},"Close");
...
*/
store.CityTownSearchUpdate = function(){
	if (getNodeAttribute('CityTownSearchBox','class') == 'dialogbox') {
		...
		var d = postJSON('RegistrationCityTownSearch',postVars);
		d.addCallbacks(store.RenderCityTownSearchResults,store.error_report);
	}
}
/*
	Close the address city/town search box
*/
store.CloseCityTownSearch = function() {
...
store.CityTownSearchKeydown = function(dom_obj){
	...
	//Process the even
	if (dom_obj.key()['string']=='KEY_ENTER') {
		store.SelectCityTownResult(dom_obj);
...
*/
store.RenderCityTownSearchResults = function(data){
	...
	for (i=0; i<Inputs.length; i++) {
		if (getNodeAttribute(Inputs[i],'name')=='btnSearchResultItem') {
			connect(Inputs[i],'onclick',store.SelectCityTownResult);
		}
	}
}
/*
...
*/
store.SelectCityTownResult = function(dom_obj) {
...
*/
store.RenderRooms = function(data){
...
connect(window, 'onload', function(){
		if (getElement("CityTownName")!=null) {
			connect("CityTownName",'onkeydown',store.CityTownSearchKeyDown);
		}
		if (getElement("btnCityTown")!=null) {
			connect("btnCityTown",'onclick',store.RenderCityTownSearch);
		}
		if (getElement("PostOffice")!=null) {
			connect("PostOffice",'onkeydown',store.CityTownSearchKeyDown);
		}
		if (getElement("Block")!=null) {
			connect("Block",'onkeydown',store.CityTownSearchKeyDown);
		}
		if (getElement("District")!=null) {
			connect("District",'onkeydown',store.CityTownSearchKeyDown);
		}
		if (getElement("State")!=null) {
			connect("State",'onkeydown',store.CityTownSearchKeyDown);
		}
		if (getElement("Country")!=null) {
			connect("Country",'onkeydown',store.CityTownSearchKeyDown);
		}
		if (getElement("DateBirth")!=null) {
			connect("DateBirth",'onclick',store.DatePick);
			connect("DateBirth",'onkeydown',store.DatePick);
			connect("DateBirth",'onblur',store.DatePickUpdate);
			store.DateBirth = getElement("DateBirth").value;
		}
		if (getElement("Age")!=null) {
			connect("Age",'onblur',store.AgePickUpdate);
			store.Age = getElement("Age").value;
		}
		if (getElement("Ward")!=null) {
			connect("Ward",'onchange',store.WardOnChange);
			connect("Room",'onchange',store.RoomOnChange);
		}
	});
...
*/
store.customeriddialog_remove = function(){
	swapDOM('customerid_dialog',null);
	swapDOM('customerid_shadow',null);
}
store.renderCustomerIdDialog = function(){
	...
	//Close link
	var close_link = createDOM('A',{'href':'javascript:store.customeriddialog_remove()'},"Close");
	dialog.appendChild(close_link);
	//The main table
