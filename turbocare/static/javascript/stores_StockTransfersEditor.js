...
store.QuickSearch = function(dom_obj){
	if ((dom_obj.key()['string']=='KEY_ENTER')&&(getElement('QuickSearch').value!='')) {
		...
		var d = postJSON('StockTransfersEditorQuickSearch',postVars);
...
store.FkSelect = function(dom_obj) {
	...
	if (dom_obj.src().id == 'btnPickStockItemID') {
		store.FkDivID = 'StockItemID';
		if (getElement('StockTransferRequestItemID')!=null) {
			var UrlVars = 'StockTransferRequestItemID='+getElement('StockTransferRequestItemID').value;
		} else {
			var UrlVars = null;
		}
		var d = postJSON('StockTransfersEditorStockItemSelect',UrlVars);
	} else if (dom_obj.src().id == 'btnPickToLocationID') {
		store.FkDivID = 'ToLocationID';
		var d = postJSON('StockTransfersEditorToLocationSelect',null);
	}
...
store.CancelChanges = function(dom_obj){
	location = "StockTransfersEditor?StockTransferID="+getElement("StockTransferID").value;
}
/*
...
store.RenderQuickSearch = function(data){
	...
	for (i=0; i<results.length; i++) {
		QuickSearchResults.appendChild(AddResultRow('StockTransfersEditor','StockTransferID='+results[i].id,results[i].text));
	}
}
/*
...
connect(window, 'onload', function(){
		...
		if (getElement("btnPickStockItemID")!=null){
			connect("btnPickStockItemID",'onclick',store.FkSelect);
		}
		if (getElement("btnPickToLocationID")!=null){
			connect("btnPickToLocationID",'onclick',store.FkSelect);
		}
});
