...
store.QuickSearch = function(dom_obj){
	if ((dom_obj.key()['string']=='KEY_ENTER')&&(getElement('QuickSearch').value!='')) {
		...
		var d = postJSON('QuoteRequestsEditorQuickSearch',postVars);
...
store.CancelChanges = function(dom_obj){
	location = "QuoteRequestsEditor?QuoteRequestID="+getElement("QuoteRequestID").value;
}
/*
...
store.AddItems = function(dom_obj){
	var ID = getElement('QuoteRequestID').value;
	pick.openPickList("QuoteRequestsEditorAddItemsStep1?QuoteRequestID="+ID);
}
/*
	Vendors
	Similar to any related join type selection
*/
store.Vendors = function(dom_obj){
	store.toggle_message("Loading...");
	var postVars = 'QuoteRequestID='+getElement('QuoteRequestID').value;
	// The following line needs to point to the function which is used for the quick search in your context
	var d = postJSON('QuoteRequestsEditorVendorSelect',postVars);
	d.addCallbacks(store.RenderVendors,store.error_report);
}
...
store.RenderQuickSearch = function(data){
	...
	for (i=0; i<results.length; i++) {
		QuickSearchResults.appendChild(AddResultRow('QuoteRequestsEditor','QuoteRequestID='+results[i].id,results[i].text));
	}
}
/* 	
...
*/
//Render the box
store.RenderVendors = function(data){
	var AddResultRow = function(value){
		//Each row needs: id, text, and selected
		var row = createDOM('DIV',{'class':'divtable'});
		if (value.selected) {
			var Check = createDOM('INPUT',{'type':'checkbox', 'checked':'checked','value':value.id,'name':'VendorID'});
		} else {
			var Check = createDOM('INPUT',{'type':'checkbox','value':value.id,'name':'VendorID'});
		}
		var text = createDOM('INPUT',{'id':'VendorName'+value.id,'type':'text','readonly':'readonly','value':value.text,'name':'VendorName'});
	...
	//Close link
	var close_link = createDOM('A',{'href':'javascript:store.Vendors_remove()'},"Close");
	...
	btnRow.appendChild(createDOM('INPUT',{'name':'btnSelectVendors','id':'btnSelectVendors','type':'BUTTON','value':'OK'}));
	dialog.appendChild(btnRow);
	//Attach an event to the button
	connect('btnSelectVendors','onclick',store.VendorsSelect);
}
store.Vendors_remove = function(){
	swapDOM('relatedjoin_dialog',null);
	swapDOM('relatedjoin_shadow',null);
}
store.VendorsSelect = function(dom_obj){
	var dialog = getElement('relatedjoin_dialog');
	var vendors = getElement('Vendors');
	//Clear our current groups listing
	replaceChildNodes(vendors,null);
	var checkboxes = getElementsByTagAndClassName('INPUT',null,dialog);
	// For every input box line, append an entry
	for (i=0; i<checkboxes.length;i++) {
		if (checkboxes[i].checked) {
			var text = getElement('VendorName'+checkboxes[i].value).value;
			var value = checkboxes[i].value;
			vendors.appendChild(createDOM('LI',null,text));
			vendors.appendChild(createDOM('INPUT',{'name':'Vendors','type':'hidden','value':value}));
			vendors.appendChild(createDOM('INPUT',{'name':'VendorsCounter','type':'hidden','value':'1'}));
		}
	}
	//Close the dialog
	store.Vendors_remove();
}
/*
	RenderAddItems:	
...
store.RenderAddItems = function(data){
	...
	var Row = function(d){
		...
		var Delete = createDOM('INPUT',{'type':'button','name':'PODelete'+d.ItemID,'value':'Delete','class':'delItem'});
		delCol.appendChild(Delete);
		replaceChildNodes(row,ROCol(d.ItemName,'ItemID',d.ItemID), Col('ItemQty',d.ItemQty),
			Col('ItemNotes',d.ItemNotes),	delCol);
...
connect(window, 'onload', function(){
		...
		if (getElement("btnEditVendors")!=null) {
			connect("btnEditVendors",'onclick',store.Vendors);
		}
