...
shortcuts.keydown = function(dom_obj){
	if (dom_obj.key()['string']=='KEY_ENTER') {
		...
		if ((ID != null) && (ID.value != null) && (ID.value != '')){
			...
			var postVars = 'GroupID='+ID.value;
			document.location.href = 'CatalogItemGroupsEditor?'+postVars;
		}
	}
}
...
config.QuickSearch = function(dom_obj){
	if ((dom_obj.key()['string']=='KEY_ENTER')&&(getElement('QuickSearch').value!='')) {
		...
		var d = postJSON('CatalogItemGroupsEditorQuickSearch',postVars);
...
config.RenderQuickSearch = function(data){
	...
	for (i=0; i<results.length; i++) {
		QuickSearchResults.appendChild(AddResultRow('CatalogItemGroupsEditor','GroupID='+results[i].id,results[i].text));
	}
}
...
config.renderIdDialog = function(){
	...
	tbody.appendChild(StringEdit('GroupID','Group ID',''));
	//Create our dialog
