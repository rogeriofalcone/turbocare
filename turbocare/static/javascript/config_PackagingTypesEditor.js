...
shortcuts.keydown = function(dom_obj){
	if (dom_obj.key()['string']=='KEY_ENTER') {
		...
		if ((ID != null) && (ID.value != null) && (ID.value != '')){
			...
			var postVars = 'PackagingID='+ID.value;
			document.location.href = 'PackagingTypesEditor?'+postVars;
		}
	}
}
...
config.QuickSearch = function(dom_obj){
	if ((dom_obj.key()['string']=='KEY_ENTER')&&(getElement('QuickSearch').value!='')) {
		...
		var d = postJSON('PackagingTypesEditorQuickSearch',postVars);
...
config.RenderQuickSearch = function(data){
	...
	for (i=0; i<results.length; i++) {
		QuickSearchResults.appendChild(AddResultRow('PackagingTypesEditor','PackagingID='+results[i].id,results[i].text));
	}
}
...
*/
config.Groups = function(e){
	config.toggle_message("Loading...");
	var postVars = 'PackagingID='+getElement('PackagingID').value;
	var d = postJSON('PackagingTypesEditorGroupSelect',postVars);
	d.addCallbacks(config.RenderGroups,config.error_report);
}
/* 	
	RenderGroups box functions
*/
//Render the box
config.RenderGroups = function(data){
	...
	//Close link
	var close_link = createDOM('A',{'href':'javascript:config.Groups_remove()'},"Close");
	...
	connect('btnSelectGroups','onclick',config.GroupsSelect);
}
config.Groups_remove = function(){
	swapDOM('relatedjoin_dialog',null);
	swapDOM('relatedjoin_shadow',null);
}
config.GroupsSelect = function(dom_obj){
	...
	for (i=0; i<checkboxes.length;i++) {
		if (checkboxes[i].checked) {
			...
			groups.appendChild(createDOM('INPUT',{'name':'Groups','type':'hidden','value':value}));
		}
	}
	//Close the dialog
	config.Groups_remove();
}
//custom short-cuts
...
connect(window, 'onload', function(){
	...
	if (getElement('btnEditGroups')!=null){
		connect('btnEditGroups','onclick',config.Groups);
	}
});
...
config.renderIdDialog = function(){
	...
	tbody.appendChild(StringEdit('PackagingID','Packaging ID',''));
	//Create our dialog
