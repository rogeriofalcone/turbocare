...
shortcuts.keydown = function(dom_obj){
	if (dom_obj.key()['string']=='KEY_ENTER') {
		...
		if ((ID != null) && (ID.value != null) && (ID.value != '')){
			...
			var postVars = 'FirmID='+ID.value;
			document.location.href = 'InsuranceEditor?'+postVars;
		}
	}
}
...
config.QuickSearch = function(dom_obj){
	if ((dom_obj.key()['string']=='KEY_ENTER')&&(getElement('QuickSearch').value!='')) {
		...
		var d = postJSON('InsuranceEditorQuickSearch',postVars);
...
config.RenderQuickSearch = function(data){
	...
	for (i=0; i<results.length; i++) {
		QuickSearchResults.appendChild(AddResultRow('InsuranceEditor','FirmID='+results[i].id,results[i].text));
	}
}
/*
...
config.renderIdDialog = function(){
	...
	tbody.appendChild(StringEdit('FirmID','Firm ID',''));
	//Create our dialog
