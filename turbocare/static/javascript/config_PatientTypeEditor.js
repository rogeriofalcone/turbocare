...
shortcuts.keydown = function(dom_obj){
	if (dom_obj.key()['string']=='KEY_ENTER') {
		...
		if ((ID != null) && (ID.value != null) && (ID.value != '')){
			...
			var postVars = 'PatientTypeID='+ID.value;
			document.location.href = 'PatientTypeEditor?'+postVars;
		}
	}
}
...
config.QuickSearch = function(dom_obj){
	if ((dom_obj.key()['string']=='KEY_ENTER')&&(getElement('QuickSearch').value!='')) {
		...
		var d = postJSON('PatientTypeEditorQuickSearch',postVars);
...
config.RenderQuickSearch = function(data){
	...
	for (i=0; i<results.length; i++) {
		QuickSearchResults.appendChild(AddResultRow('PatientTypeEditor','PatientTypeID='+results[i].id,results[i].text));
	}
}
/*
...
config.renderIdDialog = function(){
	...
	tbody.appendChild(StringEdit('PatientTypeID','Patient Type ID',''));
	//Create our dialog
