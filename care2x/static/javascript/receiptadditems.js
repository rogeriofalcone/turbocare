...
inv.loadLocations = function(id){
	if (hasParent("listing_row_"+id,"ReceiptAddItems") != true) {
		...
		inv.toggle_message("Loading...");
	  	var d = postJSON("ReceiptGetLocationsForItem","CatalogItemID="+CatalogItemID+"&LocationID="+LocationID);
...
inv.showReceiptItemAdd = function(id){
	var receiptadditems = getElement("ReceiptAddItems");
	//first, hide all other lines
	var elems = getElementsByTagAndClassName(null,"listingrowcontain_show",receiptadditems);
	if (elems.length > 0) {
		for (var i=0;i<elems.length;i++){
...
inv.makeItem = function(){
	...
	if ((location == null) || (stockrequest == null)){
		alert("First, choose a requested item and a location.");
	} else {
		//look for location in purchase order list
		var receiptitemadd_old = getElement("receiptitemadd_"+location.id+"_"+stockrequest.id);
		var linkdiv = createDOM('DIV',{'id':'closelink_'+location.id+"_"+stockrequest.id, 'class':'listingrow'});
		var closelink = createDOM('A',{'href':'javascript:inv.removeItem("receiptitemadd_'+location.id+'_'+stockrequest.id+'")'}, "Remove");
		linkdiv.appendChild(closelink);
		var checkdiv = createDOM('DIV',{'id':'checkbox_'+location.id+"_"+stockrequest.id, 'class':'listingrow'});
		var checkbox = createDOM('INPUT',{'name':'IsPaid', 'type':'checkbox', 'checked':'checked'}, "Paid");
		appendChildNodes(checkdiv,checkbox,"Paid");
		...
		appendChildNodes(divtitle,"Purchase ", qtytransfer," of " + Product + " from " + LocationName);
		var receiptadditem = createDOM('DIV',{'id':"receiptitemadd_"+location.id+"_"+stockrequest.id, 'class':'listingrowcontain', 'onclick':'inv.showReceiptItemAdd("receiptitemadd_'+location.id+'_'+stockrequest.id+'")'},divtitle);
		var clone = stockrequest.cloneNode(true);
		clone.id = clone.id + "_copy";
		appendChildNodes(receiptadditem,checkdiv,linkdiv,clone,location, counter);
		receiptadditems.appendChild(receiptadditem);
		//Any existing entry will be replaced by this one
		if (getNodeAttribute(receiptitemadd_old,"class") != null) {
			swapDOM(receiptitemadd_old, receiptadditem);
		}
		//show our current po vendor line
		inv.showReceiptItemAdd('receiptitemadd_'+location.id+'_'+stockrequest.id);
		inv.clearLocations();
	}
}
