...
inv.loadLocations = function(id){
	if (hasParent("listing_row_"+id,"StockTransfers") != true) {
		...
		inv.toggle_message("Loading...");
	  	var d = postJSON("StockTransferGetLocationsForItem","StockTransferRequestItemID="+id);
...
inv.showStockTransfer = function(id){
	var stocktransfers = getElement("StockTransfers");
	//first, hide all other shown non-lines
	var elems = getElementsByTagAndClassName(null,"listingrowcontain",stocktransfers);
	if (elems.length > 0) {
		for (var i=0;i<elems.length;i++){
			var sub_elems = getElementsByTagAndClassName('DIV',null, elems[i]);
			for (var j=0;j<sub_elems.length;j++){
				if (getNodeAttribute(sub_elems[j],'id') != 'title') {
					setNodeAttribute(sub_elems[j],'style','display:none');
				}
			}
		}
	}
	//then, hide all other shown lines
	var elems = getElementsByTagAndClassName(null,"listingrowcontain_show",stocktransfers);
	if (elems.length > 0) {
		for (var i=0;i<elems.length;i++){
...
inv.makeItem = function(){
	...
	//get the request
	var elems = getElementsByTagAndClassName(null,"listingrow_item",document);
	stockrequest = elems[0];
	for (var i=0;i<elems.length;i++){
		swapElementClass(elems[i],"listingrow_item","listingrow");
	}
	if ((location == null) || (stockrequest == null)){
		alert("First, choose a request item and a location.");
	} else {
		//look for location in purchase order list
		var stocktransfer_old = getElement("stocktransfer_"+location.id+"_"+stockrequest.id);
		var linkdiv = createDOM('DIV',{'id':'closelink_'+location.id+"_"+stockrequest.id, 'class':'listingrow'});
		var closelink = createDOM('A',{'href':'javascript:inv.removeItem("stocktransfer_'+location.id+'_'+stockrequest.id+'")'}, "Remove");
		...
		for (i=0;i<elems.length;i++){
			if (elems[i].name == "FromLocation"){
				var FromLocation = elems[i].value;
			}
		...
		elems = getElementsByTagAndClassName("input",null,stockrequest);
		for (i=0;i<elems.length;i++){
			if (elems[i].name == "ForLocation"){
				var ForLocation = elems[i].value;
			}
			if (elems[i].name == "QtyRequested"){
				var QtyRequested = elems[i].value;
			}
		}
		//Calculate the default amount to transfer from the selected location
		if (parseInt(QtyRequested)>parseInt(QtyAvailable)){
		...
		var divtitle = createDOM('DIV',{'id':'title'});
		appendChildNodes(divtitle,"Transfer ", qtytransfer," of " + Product + " from " + FromLocation + " to " + ForLocation);
		var stocktransfer = createDOM('DIV',{'id':"stocktransfer_"+location.id+"_"+stockrequest.id, 'class':'listingrowcontain', 'onclick':'inv.showStockTransfer("stocktransfer_'+location.id+'_'+stockrequest.id+'")'},divtitle);
		var clone = stockrequest.cloneNode(true);
		clone.id = clone.id+'_copy';
		setNodeAttribute(clone,'ondblclick',"inv.loadLocations('"+clone.id+"_copy')");
		appendChildNodes(stocktransfer,linkdiv,clone,location, counter);
		stocktransfers.appendChild(stocktransfer);
		//Any existing entry will be replaced by this one
		if (getNodeAttribute(stocktransfer_old,"class") != null) {
			swapDOM(stocktransfer_old, stocktransfer);
		}
		//show our current po vendor line
		inv.showStockTransfer('stocktransfer_'+location.id+'_'+stockrequest.id);
		inv.clearLocations();
	}
}
...
inv.renderLocations = function(data){
	...
	var locationDiv = function(record){
		...
		var fromlocation = createDOM('INPUT',{'type':'hidden', 'name':'FromLocation', 'value':record.Name});
		...
		appendChildNodes(tbody, tr("Name",record.Name + ", " + record.Description), tr("Quantity",record.LocationQty), tr("Product",record.Product), tr("Expire date",record.ExpireDate));
		table.appendChild(tbody);
		appendChildNodes(div,stocklocationid,fromlocation,product,qtyavailable,table);
		return div;
	}
	inv.toggle_message();
	var locations = createDOM('DIV',{'id':'Locations', 'style':'overflow:auto;'});
