...
inv.currentPanel = 'browse';
inv.multipleJoinContext = {};
inv.history = [];
inv.disclosedRelations = {};
...
inv.retrieveModels = function()
{
  var d = loadJSONDoc('list');
  d.addCallback(catwalk.renderModels);
}

inv.renderModels = function(result)
{
  if(inv.error_result(result)) return;
  var models = UL({'id':'object_list'});
  for(var i=0;i < result['SQLObjects'].length;i++)
  {
    objectName = result['SQLObjects'][i];
    var model = TD(null,A({'id':'list_'+ objectName,'href':"javascript:inv.displayObject('"+ objectName +"')"}, objectName));
...
inv.endModelDrag = function(item)
{
  var group = item.toolManDragGroup
  var list = group.element.parentNode
  var id = list.getAttribute("id")
  if (id == null) return;
  group.register('dragend', function() { inv.saveModelOrder(junkdrawer.serializeList(list)) });
}

inv.saveModelOrder = function(list)
{
  var d = loadJSONDoc('saveModelOrder?models='+list);
}
//=======================================================================================
inv.renderBoolField = function(column)
{
  var value = inv.getFieldValue(column); // getFieldValue return '' for default=False which works ok, but its a bit unexpected though.
...
inv.renderDate = function(fieldName,date_value,isDateTime)
{
  ...
  return SPAN({'class':'date_time_widget'},
               createDOM('INPUT',{'type':'text','name':fieldName,'value':date_value,'id':fieldName,'readonly':'1'}),
...
inv.displayObject = function(objectName)
{
  switch(inv.currentPanel)
  {
    case 'structure':
      inv.retrieveStructure(objectName);
      break;
    case 'browse':
      inv.retrieveValues(objectName);
      break;
    case 'add':
      inv.retrieveFormAdd(objectName);
      break;
    case 'search':
      inv.retrieveSearch(objectName);
      break;
  }
  inv.hightlightObject(objectName);
}
inv.hightlightObject = function(objectName)
{
...
inv.retrieveStructure = function(objectName)
{
  inv.currentPanel='structure';
  var d = loadJSONDoc('columns?objectName='+objectName);
  d.addCallback(inv.renderStructure);
}

inv.renderStructure = function(result)
{
  if(inv.error_result(result)) return;
  ...
  var s = DIV(null,
                inv.renderObjectMenu(result['objectName']),
                UL({'id':result['objectName'],'class':'boxy'},
                   map(inv.renderColumn, result['columns'])
                  )
              );
  ...
  {
    inv.retrieveColumnsForLabel(result['objectName'],
                                    result['reordering']['foreignObjectName'],
                                    result['reordering']['foreignKeyName']);
  }
  inv.menuSelect('structure');
}
inv.menuSelect = function(menu_item)
{
  document.getElementById('cell_'+ menu_item).style.backgroundColor='#999';
  document.getElementById('link_'+ menu_item).style.color='#fff';
}
inv.endDrag = function(item)
{
  var group = item.toolManDragGroup
  var list = group.element.parentNode
  var id = list.getAttribute("id")
  if (id == null) return;
  group.register('dragend', function() { inv.saveOrder(id,junkdrawer.serializeList(list)) });
}
inv.saveOrder = function(objectName,list)
{
  var d = loadJSONDoc('columnOrder?objectName='+ objectName +'&cols='+list);
}
//========================================================================HERE=========
inv.renderObjectMenu = function(objectName)
{
  return  DIV
          (
            null,
...
catwalk.retrieveObjectCreate = function(objectName,whereToGo)
{
  ...
  //d.addCallback((whereToGo=='save')? catwalk.renderDisplayObject:catwalk.retrieveContinueFormAdd);
